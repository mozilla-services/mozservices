...
"""
SQL database access helpers.

This module implements a thin data access layer on top of an SQL database,
building on the already-excellent facilities provided by SQLAlchemy to give
a simplified db access API.  It provides:

    * simplified resource handling and automatic cursor cleanup
    * safe automatic retry after certain types of connection failure
    * termination of interrupted queries in the mysql backend
    * annotating queries with metadata comments

The connect to the database, create a DBClient object which encapsulates
the connection pool and provides some related default behaviours:

    client = DBClient("sqlite:////tmp/test.db", pool_size=10)

You can then execute individual queries using helper methods that do resource
cleanup for you:

    print client.query_scalar("SELECT COUNT(*) FROM table")

    for name in client.query_fetchall("SELECT name FROM table"):
        print name

To execute multiple queries inside a transaction, use a DBConnection object
like this:

    with client.connect() as conn:
        conn.query('INSERT INTO table VALUES ("name1")')
        conn.query('UPDATE table SET name="name2" WHERE name="name1"')
...
class DBClient(object):
    """Database client class for SQL access layer.
    ...
    on top of the SQLAlchemy engine/connection machinery that provides some
    handy mozilla-specific helpers.
    """

    def __init__(self, sqluri, pool_size=10, no_pool=False, pool_recycle=60,
                 reset_on_return=True, pool_max_overflow=10,
                 pool_max_backlog=-1, pool_timeout=30, shard=False,
                 shardsize=100, **kwds):
    ...
    def connect(self, *args, **kwds):
        """Create a new DBConnection object from this client."""
        return DBConnection(self)

    def prepare_query(self, query, params=None, annotations=None):
        """Prepare a query for execution in the db.

        This is a customization hook for subclasses that might like to
        e.g. load queries by name from a file rather than hard-coding
...
def report_backend_errors(func):
    ...
    def report_backend_errors_wrapper(self, *args, **kwds):
        try:
            ...
            if not is_operational_db_error(self._client.engine, exc):
                raise
...
class DBConnection(object):
    ...
    higher-level utility methods for running queries with automatic cleanup.
    ...
    def execute(self, query, params=None, annotations=None):
        ...
        if connection is None:
            connection = self._client.engine.connect()
        ...
        try:
            ...
            try:
                ...
                if not is_retryable_db_error(self._client.engine, exc):
                    raise
    ...
    def _exec_with_cleanup(self, connection, query_str, **params):
        ...
        try:
            ...
            try:
                ...
                cleanup_conn = self._client.engine.pool._create_connection()
                try:
    ...
    def _render_query(self, query, params, annotations):
        ...
        if isinstance(query, basestring):
            ...
            dialect = self._client._render_query_dialect
        ...
        if annotations:
            ...
            if self._client.driver == "sqlite":
                query_str = query_str + " " + comment
            else:
                query_str = comment + " " + query_str
        return query_str

    def query(self, query, params=None, annotations=None):
        """Execute a database query, returning the rowcount."""
        if params is None:
            params = {}
        if annotations is None:
            annotations = {}
        query = self._client.prepare_query(query, params, annotations)
        if query is None:
            return 0
    ...
    def query_scalar(self, query, params=None, default=None, annotations=None):
    ...
    def query_fetchone(self, query, params=None, annotations=None):
    ...
    def query_fetchall(self, query, params=None, annotations=None):
    ...
    def insert_or_update(self, table, items, defaults=None, annotations=None):
        ...
        Given a table and a list of data dicts to insert or update, this
        method performs the "upsert" in the most efficient way.  It's a
        separate method because the precise details of the operation depend
    ...
    def _upsert_onduplicatekey(self, table, items, defaults, annotations):
        ...
        for batch in batches.itervalues():
            ...
            rowcount = self.query(query, params, annotations)
            ...
            num_updated = rowcount - len(batch)
